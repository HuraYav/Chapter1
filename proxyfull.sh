...
show_header() {
    ...
    echo -e "${NC}"
    echo -e "${GREEN}------------------------------------------------"
    echo "Наши контакты: @forthepeopl3"
    echo -e "------------------------------------------------${NC}"
}
...
show_final_message() {
    ...
    echo -e "Наши контакты: @forthepeopl3"
    # Нижняя рамка
    echo -e "${GREEN}##################################################${NC}"
}
...
# Скрипт должен запускаться только с root правами
if [ "$EUID" -ne 0 ]; then
  echo "Please run as root"
  exit 1
fi

# Справочная информация по проге для юзера
...
# Значения сети по умолчанию для необязательных аргументов
subnet=${subnet:-64}
...
# Имя глобального сетевого интерфейса
...
# Определите все необходимые пути к скриптам/конфигурациям/и т.д.
bash_location="$(which bash)"
# Получить абсолютный путь к домашнему каталогу пользователя
cd ~
user_home_dir="$(pwd)"
# Путь к каталогу со всей информацией о прокси
proxy_dir="$user_home_dir/proxyserver"
# Путь к файлу с конфигом для прокси-сервера обратного подключения
proxyserver_config_path="$proxy_dir/3proxy/3proxy.cfg"
# Путь к файлу с информацией о работающем прокси-сервере в удобном для пользователя формате.
proxyserver_info_file="$proxy_dir/running_server.info"
# Путь к файлу со всеми результирующими (внешними) адресами ipv6
random_ipv6_list_file="$proxy_dir/ipv6.list"
# Путь к файлу со случайными именами пользователей и паролями прокси
random_users_list_file="$proxy_dir/random_users.list"
# Определите правильный путь к файлу со списком прокси-серверов обратного подключения, если он не определен пользователем.
if [[ $backconnect_proxies_file == "default" ]]; then backconnect_proxies_file="$proxy_dir/backconnect_proxies.list"; fi;
# Скрипт при запуске сервера (генерация случайных идентификаторов и запуск прокси-демона)
startup_script_path="$proxy_dir/proxy-startup.sh"
# Путь конфигурации Cron (запуск прокси-сервера после перезагрузки Linux и ротации IP-адресов)
cron_script_path="$proxy_dir/proxy-server.cron"
# Последний открытый порт для прокси-сервера обратного подключения
last_port=$(($start_port + $proxy_count - 1));
# Учетные данные прокси — имя пользователя и пароль, разделенные знаком «:», если они существуют, или пустая строка, если auth == false
